...
from parsing_by_maxseminfo.parser.model.SN_PCFG import SNPCFGFixedCostReward


class HNPCFGPairwise(nn.Module):
    ...
    def evaluate(self, input_target, input_pas=None, decode_type="mbr",
                 eval_wd=False, span_mask=None, **kwargs):
        rules = self.forward(input_target)
        return self.pcfg.decode(
            rules=rules, lens=input_target['seq_len'],
            viterbi=False, mbr=True, allow_grad=False, span_mask=span_mask,
        )
...
class HNPCFGFixedCostReward(HNPCFGPairwise):
    """HN-PCFG with SemInfo REINFORCE loss.

    Reuses SNPCFGFixedCostReward.loss() directly — verified safe because
    loss() only accesses self.forward(), self.pcfg._inside(), self.pcfg.sample_crf(),
    and self.pcfg.entropy(), all of which are available on HNPCFGPairwise.
    """
    loss = SNPCFGFixedCostReward.loss
