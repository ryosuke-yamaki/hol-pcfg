#!/bin/bash
# Multilingual paper-config HN-PCFG training.
#
...
#
# Configs: config/pas-grammar/<lang_dir>/hnpcfg_nt1024_t2048_rank1_seminfo_paper[_<lang>].yaml
#
# Usage:
#   bash scripts/run_multilingual_paper_10seeds.sh [en_gpu] [zh_gpu] [fr_gpu] [de_gpu]
# Defaults: 0 1 2 3
#
# Env overrides:
...
declare -A CONFIGS=(
    [en]="config/pas-grammar/english-ew-reward-tbtok-idf/hnpcfg_nt1024_t2048_rank1_seminfo_paper.yaml"
    [zh]="config/pas-grammar/chinese-ew-reward-tbtok-idf/hnpcfg_nt1024_t2048_rank1_seminfo_paper_zh.yaml"
    [fr]="config/pas-grammar/french-ew-reward-tbtok-idf/hnpcfg_nt1024_t2048_rank1_seminfo_paper_fr.yaml"
    [de]="config/pas-grammar/german-ew-reward-tbtok-idf-vocab30k/hnpcfg_nt1024_t2048_rank1_seminfo_paper_de.yaml"
)
declare -A LANGSTR=(
    [en]="english"
    [zh]="chinese"
...
LOGDIR="logs/paper_multilingual"
mkdir -p "$LOGDIR"

echo "============================================"
echo "Paper HN-PCFG multilingual training"
...
# Returns the train.py exit code.
# ---------------------------------------------------------------------------
run_one_seed() {
    local LANG_KEY="$1"
    local SEED="$2"
    ...
    local REMARK="hnpcfg-seminfo-paper-${LANG_FULL}-seed${SEED}"
    local CKPT_DIR="ckpt/paper_multilingual/${LANG_FULL}_seed${SEED}_${TS}"
...
# ---------------------------------------------------------------------------
# Sequential chain of seeds for a single slot (one of PER_GPU_PARALLEL on a GPU).
# Returns number of failed seeds.
# ---------------------------------------------------------------------------
run_slot() {
...
# ---------------------------------------------------------------------------
# Launch PER_GPU_PARALLEL slots per language; each slot handles a round-robin
# subset of SEEDS and processes them sequentially on the shared GPU.
# ---------------------------------------------------------------------------
declare -a ALL_PIDS
declare -a ALL_TAGS
...
# ---------------------------------------------------------------------------
# Wait for all slots and aggregate failures.
# ---------------------------------------------------------------------------
total_fails=0
